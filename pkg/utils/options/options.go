...
func New() *Options {
	...
	f.BoolVar(&opts.AWSEnablePodENI, "aws-enable-pod-eni", env.WithDefaultBool("AWS_ENABLE_POD_ENI", false), fmt.Sprintf("If true then instances that support pod ENI will report a vpc.amazonaws.com/pod-eni resource. Refer to Karpenter Pod ENI documentation before enabling: https://karpenter.sh/%s/aws/provisioning/#pod-eni-security-groups-for-pods", project.GetReleaseVersion()))
